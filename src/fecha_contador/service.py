...
class DateCounterService:  # Servicio central
    ...
    def list_dates(self, group: Optional[str] = None) -> List[ImportantDate]:  # Lista fechas
        ...
        data = self.storage.load()  # Carga fechas y categorias
        items = data.dates  # Trabaja con todas las fechas
        if group:  # Filtra si se indica categoria
            target = group.lower()  # Normaliza busqueda
            items = [item for item in items if item.group.lower() == target]  # Filtro por grupo
    ...
    def list_categories(self) -> List[Category]:  # Lista categorias
        ...
        data = self.storage.load()  # Carga el estado actual
        return sorted(data.categories, key=lambda category: category.name.lower())  # Ordena alfabeticamente

    def add_category(self, name: str, color: str) -> None:  # Agrega categoria
        """Agrega una nueva categoria."""
        data = self.storage.load()  # Carga categorias actuales
        if self._category_exists(name, data.categories):  # Evita duplicados
            raise CategoryAlreadyExistsError(f"Ya existe la categoria: {name}")
        data.categories.append(Category(name=name, color=color))  # Agrega nueva categoria
        self.storage.save(data)  # Guarda cambios

    def update_category_color(self, name: str, color: str) -> bool:  # Cambia color
        """Actualiza el color de una categoria existente."""
        data = self.storage.load()  # Estado actual
        updated = False  # Bandera de actualizacion
        new_categories: List[Category] = []  # Lista nueva con cambios
        for category in data.categories:
            if category.name.lower() == name.lower():  # Coincidencia por nombre
                new_categories.append(Category(name=category.name, color=color))  # Cambia color
                updated = True  # Marca cambio
            else:
                new_categories.append(category)  # Mantiene categoria intacta
        if updated:
            self.storage.save(StoredData(dates=data.dates, categories=new_categories))  # Persiste cambios
        return updated

    def remove_category(self, name: str, move_to: str = "General") -> None:  # Elimina
        """Elimina una categoria y mueve sus fechas a otra."""
        if name.lower() == "general":  # Protege categoria base
            raise CategoryProtectedError("La categoria 'General' no se puede eliminar.")

        if move_to.lower() == name.lower():  # Evita mover a la misma categoria
            raise CategoryProtectedError("La categoria destino no puede ser la misma que la eliminada.")

        move_to = self._normalize_group(move_to)  # Normaliza destino
        data = self.storage.load()  # Carga datos actuales
        if not self._category_exists(name, data.categories):  # Verifica existencia
            raise CategoryNotFoundError(f"No se encontro la categoria: {name}")

        categories = [category for category in data.categories if category.name.lower() != name.lower()]  # Filtra la eliminada
        if not self._category_exists(move_to, categories):  # Crea destino si no existe
            categories.append(Category(name=move_to))

        updated_dates = [
            self._with_group(item, move_to) if item.group.lower() == name.lower() else item  # Reasigna grupo
            for item in data.dates
        ]
        self.storage.save(StoredData(dates=updated_dates, categories=categories))  # Guarda cambios

    def add_date(self, item: ImportantDate) -> None:  # Agrega una fecha
        """Agrega una nueva fecha si el nombre no existe."""
        data = self.storage.load()  # Carga datos actuales
        if self._exists(item.name, data.dates):  # Previene duplicados
            raise DateAlreadyExistsError(f"Ya existe una fecha con el nombre: {item.name}")

        normalized_group = self._normalize_group(item.group)  # Normaliza el grupo
        if normalized_group != item.group:  # Ajusta el item si cambia el grupo
            item = self._with_group(item, normalized_group)
        data.dates.append(item)  # Agrega fecha
        if not self._category_exists(normalized_group, data.categories):  # Asegura categoria
            data.categories.append(Category(name=normalized_group))
        self.storage.save(data)  # Persiste

    def remove_date(self, name: str) -> bool:  # Elimina por nombre
        """Elimina una fecha por nombre y avisa si existia."""
        data = self.storage.load()  # Carga datos actuales
        remaining = [item for item in data.dates if item.name.lower() != name.lower()]  # Filtra por nombre
        if len(remaining) == len(data.dates):
            return False  # No se encontro

        self.storage.save(StoredData(dates=remaining, categories=data.categories))  # Guarda lista nueva
        return True  # Elimino correctamente

    def move_to_group(self, name: str, group_name: str) -> bool:  # Cambia grupo
        """Mueve una fecha a un nuevo grupo."""
        data = self.storage.load()  # Carga datos actuales
        found = False  # Bandera de coincidencia
        group_name = self._normalize_group(group_name)  # Normaliza destino
        new_items = []  # Lista con cambios
        for item in data.dates:
            if item.name.lower() == name.lower():  # Coincide por nombre
                new_items.append(self._with_group(item, group_name))  # Reasigna grupo
                found = True  # Marca encontrado
            else:
                new_items.append(item)  # Conserva item

        if found:
            if not self._category_exists(group_name, data.categories):  # Asegura categoria destino
                data.categories.append(Category(name=group_name))
            self.storage.save(StoredData(dates=new_items, categories=data.categories))  # Guarda cambios
        return found  # Devuelve si se movio

    def next_date(self, now: Optional[datetime] = None) -> Optional[UpcomingDate]:
        """Devuelve la fecha mas cercana al momento dado."""
        if now is None:  # Usa ahora si no se pasa referencia
            now = datetime.now()

        upcoming_list = self._calculate_deltas(now)  # Calcula diferencias
        if not upcoming_list:  # Sin fechas registradas
            return None

        return min(upcoming_list, key=lambda u: abs(u.delta.total_seconds()))  # Selecciona la mas cercana
    ...
    def _exists(self, name: str, items: List[ImportantDate]) -> bool:  # Comprueba duplicado
        ...
        target = name.lower()  # Normaliza busqueda
        return any(existing.name.lower() == target for existing in items)  # Verifica coincidencia

    def _category_exists(self, name: str, categories: List[Category]) -> bool:
        target = name.lower()  # Normaliza busqueda
        return any(category.name.lower() == target for category in categories)  # Verifica coincidencia

    def _with_group(self, item: ImportantDate, group_name: str) -> ImportantDate:
        return ImportantDate(  # Reconstruye para mantener inmutabilidad
            name=item.name,
            date=item.date,
    ...
    def _normalize_group(self, group_name: str) -> str:
        if not group_name or not group_name.strip():  # Fallback si el grupo es vacio
            return "General"
        return group_name.strip()  # Limpia espacios en extremos
    ...
    def _calculate_deltas(self, now: datetime) -> List[UpcomingDate]:  # Calcula diferencias
        ...
        data = self.storage.load()  # Carga todas las fechas
        return [
            UpcomingDate(item=item, delta=(item.date - now))  # Delta respecto a ahora
            for item in data.dates
        ]
